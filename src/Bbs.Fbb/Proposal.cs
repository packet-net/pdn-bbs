...
/// <c>FC EM &lt;mid≤12&gt; &lt;usize&gt; &lt;csize&gt; 0</c>. Implementations
/// send the trailing <c>0</c>; the F4HOF ABNF omits it — both are accepted
/// on parse. BPQ↔BPQ extension fields after it are tolerated and ignored
/// (spec §3.13.3; we never advertise the <c>BPQ</c> substring that elicits
/// them).
/// </summary>
/// <param name="ControlType">Two-character type: <c>EM</c> (encapsulated message) or <c>CM</c> (control message).</param>
    ...
    int CompressedSize) : Proposal
{
    ...
    public override string ToWireLine()
    {
        if (ControlType.Length != 2)
        {
            throw new FbbProtocolException($"FC type field must be 2 chars, got \"{ControlType}\" (spec §3.9).");
        }
        ...
        return UncompressedSize < 0 || CompressedSize < 0
            ? throw new FbbProtocolException("FC sizes must be non-negative.")
            : string.Create(
                CultureInfo.InvariantCulture,
                $"FC {ControlType} {Mid} {UncompressedSize} {CompressedSize} 0");
    }
    ...
    internal static FcProposal ParseLine(string trimmedLine)
    {
        ...
        {
            throw new FbbProtocolException(
                $"FC type field must be 2 chars (EM/CM), got \"{fields[1]}\" (spec §3.9).");
        }
        ...
        return new FcProposal(fields[1].ToUpperInvariant(), fields[2], usize, csize);
    }
}
