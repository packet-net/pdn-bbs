...
| `connect-test` | Validate each enabled partner's connect script + tighten its `EXPECT=` strings from the real prompt, **moving no mail**, via the sysop `test-connect` tool (`POST /apps/bbs/forwarding/test-connect`). Direct-BBS partners need only connect; the multi-hop ones (GB7LOX `… bbs`, GB7CIP `C GB7WEM-7`→`C uhf gb7cip`) need real expects set from the observed prompt. Run after `network`, before `golive`. | n/a |
