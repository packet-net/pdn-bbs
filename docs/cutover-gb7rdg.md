...
- **CT staged + held**: `node-v0.22.0`+ (run the current `node-v0.26.0`) and **`pdn-bbs 0.2.52`+** (connect-scripts v2 — structured steps + the live test-connect editor; older builds can't author the multi-hop partners' scripts) installed; the GB7RDG node config + `bbs.yaml` applied and fully HELD (ports `enabled:false`, `oarc.enabled:false`, `forwarding.enabled:false`, `housekeeping` set to BPQ's `MaxAge` — all `*Days: 365`, `bidLifetimeDays: 60`). `preflight` asserts all of this.
