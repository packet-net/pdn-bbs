...
- **CT staged + held**: `node-v0.22.0`+ (run the **current release** — `node-v0.35.0` as of 2026-07-20; 0.35.0 adds a `libhamlib-utils` dependency that `apt-get -f install` resolves, and migrates the persisted config schema v1→v2 on first start — the held `schemaVersion: 1` YAML still imports cleanly) and **`pdn-bbs 0.2.52`+** (connect-scripts v2 — structured steps + the live test-connect editor; older builds can't author the multi-hop partners' scripts) installed; the GB7RDG node config + `bbs.yaml` applied and fully HELD (ports `enabled:false`, `oarc.enabled:false`, `forwarding.enabled:false`, `housekeeping` set to BPQ's `MaxAge` — all `*Days: 365`, `bidLifetimeDays: 60`). `preflight` asserts all of this.
