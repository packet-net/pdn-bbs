...
This is **one-way**. Once the new node forwards mail on-air, the network has advanced past stopped LinBPQ and there is **no rollback**. So the cutover is split into phases you run **one at a time**, each verifying before it returns, with a **safe-abort window** up to (but not including) `golive`. Nothing before `golive` moves mail — forwarding is gated in **two tiers** (the whole-BBS **master switch**, held; AND every partner imported **disabled**), so both must open before anything forwards. `connect-test` is where you test + **enable each partner**; `golive` flips the master ON (and refuses unless ≥1 partner is enabled, else it would dial no one).
...
- **CT staged + held**: `node-v0.22.0`+ (run the **current release** — `node-v0.36.1` as of 2026-07-21 — the connect-script Dial `port` is a port **id/name** (e.g. `hf-40m`), and it carries the #664 (pacing retry on KISS-TCP reconnect) / #665 (no silent first-port) fixes; note 0.35.0+ adds a `libhamlib-utils` dependency that `apt-get -f install` resolves, and migrates the persisted config schema v1→v2 on first start — the held `schemaVersion: 1` YAML still imports cleanly) and **`pdn-bbs 0.2.53`+** (connect-scripts v2 — structured steps + the live test-connect editor; older builds can't author the multi-hop partners' scripts) installed; the GB7RDG node config + `bbs.yaml` applied and fully HELD (ports `enabled:false`, `oarc.enabled:false`, `forwarding.enabled:false`, `housekeeping` set to BPQ's `MaxAge` — all `*Days: 365`, `bidLifetimeDays: 60`). `preflight` asserts all of this.
...
| `connect-test` | The per-partner **bring-up**. Every partner imports **disabled with a _blank_ connect script** — connect-scripts v2 retired the flat-form auto-translate ([`connect-script-v2.md`](connect-script-v2.md)) — so here you **author each partner's structured connect script**, `Test connect` it (moving **no mail**), then **Enable** the good ones, in the BBS sysop UI (Forwarding page) or via `POST /apps/bbs/forwarding/{test-connect,partner/enable}`. `Test connect` streams the live dialogue; **"Use as Wait-for"** drops the peer's observed prompt (trailing space and all) verbatim into a step's `expect`. Direct-BBS partners are a one-step `connect: <call>`; the multi-hop ones (GB7LOX `… bbs` via GB7LOX-2, GB7CIP `C 3 !GB7WEM-7`→URONode `=> `→`BBS`) need the full expect/send walk. A partner left disabled simply won't forward. Run after `network`, before `golive`. | n/a |
| `golive` | **POINT OF NO RETURN.** Re-runs the load-bearing readiness checks *hard* (healthz, wg address, modem links, currently-held) **plus ≥1 partner enabled** (else forwarding would dial no one), requires typing `GB7RDG GO`, then flips the **store-backed master ON** (the actual "forwarding starts" step — the bbs.yaml flag is only a first-start seed) + enables OARC; records the golive timestamp. Mail starts moving to the enabled partners. | **NO** |
