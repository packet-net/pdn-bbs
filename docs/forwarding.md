...
- **Housekeeping lifetime defaults + `MaxMsgno`/renumber.** ✅ **shipped (issue #39).** The per-class lifetime DEFAULTS now match the GB7RDG convention — bulletins ~7 days, personals + NTS ~30 days (`HousekeepingPolicy.DefaultBulletinDays`/`DefaultPersonalDays`) — and the whole matrix is overridable per node WITHOUT a code change through the new `housekeeping:` block in `bbs.yaml` (`HousekeepingConfig.ToPolicy()`; every key optional, an absent block keeps the built-in defaults so an upgrade changes nothing). `MaxMsgno` (the BPQ message-number ceiling) drives a compacting **renumber** that runs LAST in the housekeeping pass, after kill-by-age + the K-purge have shrunk the store: when the high-water mark reaches the ceiling the survivors are renumbered densely from 1 (`BbsStore.RenumberMessages`, one transaction with FK checks deferred to COMMIT — the commit is the crash-safe boundary). Referential integrity holds because the **network BID (`<n>_<BBSCALL>`) is never rewritten** — it is a frozen network identity partners track on the wire; only the local `messages.number` and the local rows keyed on it move (recipients, forwards, attachments, per-user read state, 7plus parts/files, the `bids.message_number` back-link, and each user's last-listed watermark, mapped to the new number of the highest survivor at/below the old watermark). Default `MaxMsgno` is 0 (renumbering off). Pinned by `HousekeepingTests` (per-class age-out incl. bulletins-at-7 vs personals-at-30, MaxMsgno fire/compaction, recipient+forward+read-state integrity, watermark remap, reopen-durability) and `ConfigTests` (absent-block defaults, partial + full override, negative/zero clamp).
...
- **Receiver-side restart granting (`FS !offset`).** ✅ **shipped (issue #38).** The receiver now persists partial INBOUND transfers durably so it can grant `!offset` itself, mirroring the send side. Each received block stages the compressed prefix to a crash-safe scratch area (`FileInboundPartialStore` under `$PDN_APP_STATE/inbound-partials/`, separate from the committed store, keyed by `(peer base callsign, BID/MID)`); the commit boundary is a temp-write→fsync→atomic-rename per block, so a daemon restart finds either the prior committed partial or the new one — never a torn file — and a stale partial is GC'd after a TTL (default 7 days, plus an orphan-`.tmp` sweep at startup). On a re-offer of a message we hold a partial for, the FSM upgrades the host's `+` to `FS !(held−6)` and reuses the held bytes: the peer re-sends the 6-byte header preamble then the tail from that offset, and the reader reconstructs the full compressed image (the preamble-skip is now a per-byte count, robust to either a BPQ standalone-6-byte block or our packed-block framing). Guards: a partial under 7 bytes, or an FC whose advertised compressed size no longer matches, falls back cleanly to a full from-zero receive; a committed message discards its partial. Sans-IO: the FSM consults an injected `IInboundResumeStore` (peer-scoped); a null store (tests/standalone) keeps the exact pre-#38 behaviour. Pinned by `FbbSessionResumeTests` (grant-offset + byte-identical reconstruction, an interrupt→resume round-trip across the two-session harness with the real caller honouring the grant, divergence/tiny-partial/reject fall-backs) and `FileInboundPartialStoreTests` (round-trip, reopen-durability as the restart surrogate, peer+mid keying, base-callsign SSID sharing, TTL GC).
- **Inbound FBB forwarding over internet-TCP (BPQ `FBBPORT`).** ✅ **shipped (issue #40).** A raw-TCP forwarding listener (`FbbTcpListener`, the BPQ `FBBPORT` equivalent) now accepts inbound forwarding sessions from internet partners alongside the AX.25/RHP path, **sharing the FBB protocol engine** rather than duplicating it: both bearers implement `IFbbConnection` and are driven by the same `FbbSessionRunner` (and thus the one FSM). Partner identity over connectionless TCP comes from the **in-protocol FBB login handshake** (the listener prompts `Callsign :` and reads the partner's callsign — the "literal login name" BPQ uses), and that callsign is the authorization: it must match an `enabled` partner (base-call match, as the reverse-forward join is) or the connection is refused before any session starts — so the existing partner-specific forwarding policy applies by that identity (size caps, B2 gate, routing/loop guards, receiver restart-granting). Config surface (`fbbTcp:` in `bbs.yaml`, `FbbTcpConfig`): `enabled` (default **off** — an internet-facing port is opt-in), `bind` (default loopback), `port` (default 8011, BPQ's conventional FBBPORT), `maxConnections` (concurrency bound). Security posture: opt-in, conservative loopback default, a connection cap and a login-read timeout, and a rejection notice that never leaks our SID/internals to an unauthorized caller; TLS/transport hardening is left to the edge (the tailnet sidecar under pdn, as for IMAP/SMTP — a per-partner password is a possible future hardening). Pinned by `FbbTcpListenerTests` (a known partner authenticates via the callsign login + forwards a message in over a real loopback socket; unknown and disabled partners rejected with no session/message; SSID-on-login tolerated) and `ConfigTests` (defaults-off, opt-in round-trip, default-YAML parses to disabled).
