...
- **Forward timing**: `FWDTimes = ""` (no time-band restriction), `FWDNewImmediately = 1` + `FwdInterval = 2` **seconds** — the oracle dials `ConnectScript` (`C 2 PDNBBS-1`, port 2 = the netsim channel) within ~2 s of a message being queued, and keeps redialling (~30 s per failed cycle: SABM retries then back-off) until something answers as `PDNBBS-1` on the channel. A test that listens on netsim node `a` as `PDNBBS-1` gets called without any prodding; the deterministic sysop levers (`FWD PDNBBS-1 NOW`, `DOHOUSEKEEPING`, `REROUTEMSGS`) also work from an `admin` telnet session.
