...
- **Protocol: FBB compressed B1 **and** B2 ("B12F")** — `AllowBlocked=1, AllowCompressed=1, UseB1Protocol=1, UseB2Protocol=1`. With our side B1-only (the default — `AllowB2F` off) the SID intersection drops B2 and the oracle proposes FA: `F>` checksum, FS `+ - =`, SOH/STX/EOT framing, LZHUF N=2048 (spec §3.6–3.7). With our side B2-enabled (`AllowB2F = true`, the `B2ForwardingInteropTests` lane) the oracle advertises `2` in the forwarding-session SID and proposes/accepts `FC EM` (the B2 object rides the same B1 container + framing). The B1F MUST-line is unaffected — both modes verified live against this stock image.
...
### B2F oracle gate (`B2ForwardingInteropTests` — design.md decision #7)

10. **The forwarding-session SID DOES track the partner record's `UseB2Protocol`, both directions** — correcting delta 6/8's "us→oracle answers `B2FWIHJM$` regardless". Live: with `UseB2Protocol = 0` the oracle's forwarding SID to a dialling `PDNBBS-1` is `[BPQ-6.0.25.23-B1FWIHJM$]` (B1 only, no `2`); flip the `BBSForwarding.PDNBBS` record to `UseB2Protocol = 1` and it becomes `…-B12FWIHJM$`. So even the us→oracle (auto-user) direction reflects the partner gate — the SID is built from `user->ForwardingInfo->AllowB1/AllowB2` (`BBSUtilities.c` ~11000). Our side must offer B2 too (`AllowB2F = true` → our SID carries `2`); the session's `B2Active` is the intersection, and only then does the oracle propose/accept `FC EM`.
11. **LinBPQ stores an inbound B2 (FC) arrival as the RAW B2 object** — the whole §3.9 envelope (`MID:`/`Date:`/`Type:`/`From:`/`To:`/`Subject:`/`Mbo:`/`Content-*`/`Body:` count), a blank line, then the body — straight into `m_*.mes`. It does **not** strip the envelope and re-head the file with a fresh `R:` line the way it does for a B1 (FA) arrival (whose `.mes` starts at the R: chain). Our R: line still survives, but **nested inside the `Body:` section**, not leading. Assert B2 inbound storage on the header fields + body, not on a leading R: line (`B2ForwardingInteropTests.OutboundB2Cycle`).
12. **Applying `UseB2Protocol` to a RUNNING oracle needs SIGKILL, not restart.** BPQMail rewrites `/data/linmail.cfg` from its in-memory model on a *clean* shutdown, so an edit-then-`docker restart` is silently reverted at SIGTERM (verified: the value flipped back to `0`). To re-key a live container that pre-dates this seed: `docker kill pdnbbs-gb7bpq` (SIGKILL — no save), edit `/data/linmail.cfg` via a throwaway container on the `./oracle/state` mount (it's root-owned), then `docker start` (which reads the edited file). A fresh `up -d --wait` seeds the committed `UseB2Protocol = 1` directly, so CI needs none of this.
