...
public sealed class BpqImporterIntegrationTests : IDisposable
{
    ...
    public void Import_Partners_ImportedExceptSelf()
    {
        ...
        // v2: the flat connect-script form is retired — every partner imports with a BLANK connect
        // script (the sysop authors a structured one before enabling). See docs/connect-script-v2.md.
        Assert.Equal("", ScalarString(c, "SELECT connect_script FROM partners WHERE call='GB7BSK';"));
        Assert.Equal("", ScalarString(c, "SELECT connect_script FROM partners WHERE call='GB7CIP';"));
    }
