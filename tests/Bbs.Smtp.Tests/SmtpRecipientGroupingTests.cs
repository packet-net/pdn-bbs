...
/// <summary>
/// Unit tests for the pure addr→(call, at) split + route-grouping that turns the accepted RCPT list into
/// one <c>MessageDraft</c> per distinct At. No server — just the static helper.
/// </summary>
public sealed class SmtpRecipientGroupingTests
{
    ...
    {
        IReadOnlyList<SmtpRecipientGroup> groups = SmtpRecipientGrouping.Group(
            ["G0ABC@GB7RDG.GBR.EURO", "G0XYZ", "G0DEF@GB7RDG.GBR.EURO", "G0LOCAL"]);
        ...
        Assert.Equal(["G0XYZ", "G0LOCAL"], groups[1].Calls);
    }
